...
class ProInLogic:
    ...
    def otherProvinceMlt(data, length=96):
        ...
        # 计算中长期成本费用
        mlt_cost_fee_res = CommonCal.weightedMean(
        ...
        # 中长期*成本
        mlt_cost_fee_list = mlt_cost_fee_res["numeratorList"]

        data["mlt_ele_list"] = mlt_ele_list
        data["mlt_fee_list"] = mlt_fee_list
        data["mlt_cost_fee_list"] = mlt_cost_fee_list
    ...
    def otherProvinceDayAhead(data, length=96):
        ...
        # 日前正偏差电量*日前出清电价
        dayAhead_positive_diff_price_fee_list = dayAhead_positive_diffeleMulPrice_res["numeratorList"]
        # 日前负偏差电量*日前出清电价
        dayAhead_negative_diff_price_fee_list = dayAhead_negative_diffeleMulPrice_res["numeratorList"]


        # 日前正偏差电量*成本价计算
        dayAhead_positive_diffeleMulCost_res = CommonCal.weightedMean(
            numeratorList=[dayAhead_positive_ele_list, data["change_cost"]],
            denominatorList=[dayAhead_positive_ele_list],
            length=length
        )
        # 日前负偏差电量*成本计算
        dayAhead_negative_diffeleMulCost_res = CommonCal.weightedMean(
            numeratorList=[dayAhead_negative_ele_list, data["change_cost"]],
            denominatorList=[dayAhead_negative_ele_list],
            length=length
        )

        # 日前正偏差电量*成本
        dayAhead_positive_diff_cost_fee_list = dayAhead_positive_diffeleMulCost_res["numeratorList"]
        # 日前负偏差电量*成本
        dayAhead_negative_diff_cost_fee_list = dayAhead_negative_diffeleMulCost_res["numeratorList"]
        ...
        data["dayAhead_positive_diff_price_fee_list"] = dayAhead_positive_diff_price_fee_list
        data["dayAhead_negative_diff_price_fee_list"] = dayAhead_negative_diff_price_fee_list

        data["dayAhead_positive_diff_cost_fee_list"] = dayAhead_positive_diff_cost_fee_list
        data["dayAhead_negative_diff_cost_fee_list"] = dayAhead_negative_diff_cost_fee_list
    ...
    def otherProvinceRealTime(data, length=96):
        ...
        # 计算实时偏差电量，实时-中长期，任意一个为空，当0计算
        realTime_diffEle_cal_res = CommonCal.conductSubtract(data["real_time_ele"], data["day_ahead_ele"], A_NoneToZero=True,
                                                             B_NoneToZero=True)
        ...
        # 实时正偏差电量*实时出清电价
        realTime_positive_diff_price_fee_list = realTime_positive_diffeleMulPrice_res["numeratorList"]
        # 实时负偏差电量*实时出清电价
        realTime_negative_diff_price_fee_list = realTime_negative_diffeleMulPrice_res["numeratorList"]

        # 实时正偏差电量*成本价计算
        realTime_positive_diffeleMulCost_res = CommonCal.weightedMean(
            numeratorList=[realTime_positive_ele_list, data["change_cost"]],
            denominatorList=[realTime_positive_ele_list],
            length=length
        )
        # 实时负偏差电量*成本计算
        realTime_negative_diffeleMulCost_res = CommonCal.weightedMean(
            numeratorList=[realTime_negative_ele_list, data["change_cost"]],
            denominatorList=[realTime_negative_ele_list],
            length=length
        )

        # 实时正偏差电量*成本
        realTime_positive_diff_cost_fee_list = realTime_positive_diffeleMulCost_res["numeratorList"]
        # 实时负偏差电量*成本
        realTime_negative_diff_cost_fee_list = realTime_negative_diffeleMulCost_res["numeratorList"]
        ...
        data["realTime_positive_diff_price_fee_list"] = realTime_positive_diff_price_fee_list
        data["realTime_negative_diff_price_fee_list"] = realTime_negative_diff_price_fee_list

        data["realTime_positive_diff_cost_fee_list"] = realTime_positive_diff_cost_fee_list
        data["realTime_negative_diff_cost_fee_list"] = realTime_negative_diff_cost_fee_list

    # 其他省份分时收益数据计算
    @staticmethod
    def otherProvinceIncomeCal(dataList, length=96):
        ...
        fieldNameList = [
            ...
            "dayAhead_positive_diff_price_fee_list",
            "dayAhead_negative_diff_price_fee_list",
            "dayAhead_positive_diff_cost_fee_list",
            "dayAhead_negative_diff_cost_fee_list",
            "realTime_ele_list",
            "realTime_fee_list",
            "realTime_positive_ele_list",
            "realTime_negative_ele_list",
            "realTime_positive_diff_price_fee_list",
            "realTime_negative_diff_price_fee_list",
            "realTime_positive_diff_cost_fee_list",
            "realTime_negative_diff_cost_fee_list",

        ]

        fieldDataDict = CommonCal.conductAddMulField(fieldNameList,dataList, length=96)

        # 中长期收益计算
        # 计算中长期电价
        mlt_price_list = CommonCal.conductDivide(fieldDataDict["mlt_fee_list"], fieldDataDict["mlt_ele_list"], A_NoneToZero=True, B_NoneToZero=True)["diff"]
        # 计算变动成本
        mlt_cost_list = CommonCal.conductDivide(fieldDataDict["mlt_cost_fee_list"], fieldDataDict["mlt_ele_list"], A_NoneToZero=True, B_NoneToZero=True)["diff"]
        # 中长期电价-变动成本，中长期为空时，变动成本为空时，当0计算
        mlt_diff_price = CommonCal.conductSubtract(mlt_price_list, mlt_cost_list, A_NoneToZero=True, B_NoneToZero=True)["diff"]
        # 计算中长期收益
        mlt_income_res = CommonCal.weightedMean(
            numeratorList=[fieldDataDict["mlt_fee_list"], mlt_diff_price],
            denominatorList=[fieldDataDict["mlt_fee_list"]],
            length=length
        )
        ...
        # 日前正偏差收益
        # 计算日前正偏差量*出清价/量
        dayAhead_positive_price_list = \
        CommonCal.conductDivide(fieldDataDict["dayAhead_positive_diff_price_fee_list"], fieldDataDict["dayAhead_positive_ele_list"], A_NoneToZero=True,
                                B_NoneToZero=True)["diff"]
        # 计算变动成本
        dayAhead_positive_cost_list = \
        CommonCal.conductDivide(fieldDataDict["dayAhead_positive_diff_cost_fee_list"], fieldDataDict["dayAhead_positive_ele_list"], A_NoneToZero=True,
                                B_NoneToZero=True)["diff"]
        # 日前电价-变动成本，日前为空时，变动成本为空时，当0计算
        dayAhead_positive_diff_price = CommonCal.conductSubtract(dayAhead_positive_price_list, dayAhead_positive_cost_list, A_NoneToZero=True, B_NoneToZero=True)[
            "diff"]
        # 计算日前正收益
        dayAhead_positive_income_res = CommonCal.weightedMean(
            numeratorList=[fieldDataDict["dayAhead_positive_ele_list"], dayAhead_positive_diff_price],
            denominatorList=[fieldDataDict["dayAhead_positive_ele_list"]],
            length=length
        )
        dayAhead_positive_income_list = dayAhead_positive_income_res["numeratorList"]

        # 日前负偏差收益
        # 计算日前负偏差量*出清价/量
        dayAhead_negative_price_list = \
            CommonCal.conductDivide(fieldDataDict["dayAhead_negative_diff_price_fee_list"],
                                    fieldDataDict["dayAhead_negative_ele_list"], A_NoneToZero=True,
                                    B_NoneToZero=True)["diff"]
        # 计算变动成本
        dayAhead_negative_cost_list = \
            CommonCal.conductDivide(fieldDataDict["dayAhead_negative_diff_cost_fee_list"],
                                    fieldDataDict["dayAhead_negative_ele_list"], A_NoneToZero=True,
                                    B_NoneToZero=True)["diff"]
        # 日前电价-变动成本，日前为空时，变动成本为空时，当0计算
        dayAhead_negative_diff_price = \
        CommonCal.conductSubtract(dayAhead_negative_price_list, dayAhead_negative_cost_list, A_NoneToZero=True,
                                  B_NoneToZero=True)[
            "diff"]
        # 计算日前负收益
        dayAhead_negative_income_res = CommonCal.weightedMean(
            numeratorList=[fieldDataDict["dayAhead_negative_ele_list"], dayAhead_negative_diff_price],
            denominatorList=[fieldDataDict["dayAhead_negative_ele_list"]],
            length=length
        )
        dayAhead_negative_income_list = dayAhead_negative_income_res["numeratorList"]



        # 实时正偏差收益
        # 计算实时正偏差量*出清价/量
        realTime_positive_price_list = \
        CommonCal.conductDivide(fieldDataDict["realTime_positive_diff_price_fee_list"], fieldDataDict["realTime_positive_ele_list"], A_NoneToZero=True,
                                B_NoneToZero=True)["diff"]
        # 计算变动成本
        realTime_positive_cost_list = \
        CommonCal.conductDivide(fieldDataDict["realTime_positive_diff_cost_fee_list"], fieldDataDict["realTime_positive_ele_list"], A_NoneToZero=True,
                                B_NoneToZero=True)["diff"]
        # 实时电价-变动成本，实时为空时，变动成本为空时，当0计算
        realTime_positive_diff_price = CommonCal.conductSubtract(realTime_positive_price_list, realTime_positive_cost_list, A_NoneToZero=True, B_NoneToZero=True)[
            "diff"]
        # 计算实时正收益
        realTime_positive_income_res = CommonCal.weightedMean(
            numeratorList=[fieldDataDict["realTime_positive_ele_list"], realTime_positive_diff_price],
            denominatorList=[fieldDataDict["realTime_positive_ele_list"]],
            length=length
        )
        realTime_positive_income_list = realTime_positive_income_res["numeratorList"]

        # 实时负偏差收益
        # 计算实时负偏差量*出清价/量
        realTime_negative_price_list = \
            CommonCal.conductDivide(fieldDataDict["realTime_negative_diff_price_fee_list"],
                                    fieldDataDict["realTime_negative_ele_list"], A_NoneToZero=True,
                                    B_NoneToZero=True)["diff"]
        # 计算变动成本
        realTime_negative_cost_list = \
            CommonCal.conductDivide(fieldDataDict["realTime_negative_diff_cost_fee_list"],
                                    fieldDataDict["realTime_negative_ele_list"], A_NoneToZero=True,
                                    B_NoneToZero=True)["diff"]
        # 实时电价-变动成本，实时为空时，变动成本为空时，当0计算
        realTime_negative_diff_price = \
        CommonCal.conductSubtract(realTime_negative_price_list, realTime_negative_cost_list, A_NoneToZero=True,
                                  B_NoneToZero=True)[
            "diff"]
        # 计算实时负收益
        realTime_negative_income_res = CommonCal.weightedMean(
            numeratorList=[fieldDataDict["realTime_negative_ele_list"], realTime_negative_diff_price],
            denominatorList=[fieldDataDict["realTime_negative_ele_list"]],
            length=length
        )
        realTime_negative_income_list = realTime_negative_income_res["numeratorList"]

        # 日前结算收益
        dayAhead_settlement_income_list = CommonCal.conductAdd(
            [dayAhead_positive_income_list, dayAhead_negative_income_list])
        # 实时结算收益
        realTime_settlement_income_list = CommonCal.conductAdd(
            [realTime_positive_income_list, realTime_negative_income_list])

        # 日前收益
        dayAhead_income_list = CommonCal.conductAdd(
            [dayAhead_settlement_income_list, mlt_income_list])
        # 实时收益
        realTime_income_list = CommonCal.conductAdd(
            [realTime_settlement_income_list, dayAhead_income_list])
        # 现货增收
        ...
        comprehensive_income_list = CommonCal.conductAdd(
            [
                fieldDataDict["dayAhead_positive_diff_price_fee_list"],
                fieldDataDict["dayAhead_negative_diff_price_fee_list"],
                fieldDataDict["realTime_positive_diff_price_fee_list"],
                fieldDataDict["realTime_negative_diff_price_fee_list"],
                fieldDataDict["mlt_fee_list"],
            ]
        )
        # 计算综合价的费用的实时出清电量
        comprehensive_ele_list = copy.deepcopy(fieldDataDict["realTime_ele_list"])

        fieldDataDict["dayAhead_positive_income_list"] = dayAhead_positive_income_list
        fieldDataDict["dayAhead_negative_income_list"] = dayAhead_negative_income_list
        fieldDataDict["realTime_positive_income_list"] = realTime_positive_income_list
        fieldDataDict["realTime_negative_income_list"] = realTime_negative_income_list
        fieldDataDict["dayAhead_settlement_income_list"] = dayAhead_settlement_income_list
        fieldDataDict["realTime_settlement_income_list"] = realTime_settlement_income_list
        fieldDataDict["mlt_income_list"] = mlt_income_list
        fieldDataDict["dayAhead_income_list"] = dayAhead_income_list
        fieldDataDict["realTime_income_list"] = realTime_income_list
        fieldDataDict["spot_incomeIncrease_list"] = spot_incomeIncrease_list
        fieldDataDict["comprehensive_income_list"] = comprehensive_income_list
        fieldDataDict["comprehensive_ele_list"] = comprehensive_ele_list
    ...
    # 蒙西省份分时收益数据计算
    @staticmethod
    def MXProvinceIncomeCal(dataList, length=96):
